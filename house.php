...
function addFavorite(id, title) {
  ...
  xhttp.open("POST", "add_favorite.php", true);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send("id=" + id + "&title=" + title);
}
</script>
<div class="w-full h-screen">
  ...
  <div class="fixed bottom-0 w-full h-16 bg-white shadow-inner">
    <div class="max-w-screen-lg mx-auto h-full flex justify-end items-center">
      ...
      <?php endif; ?>
      <button type="button" data-tooltip-target="tooltip-default" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center ml-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-800" onclick="showInputBox()" >聯絡/評價房東</button>
      <button type="submit" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center ml-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-800" onclick="addFavorite(<?php echo $house['id']; ?>, '<?php echo $house['title']; ?>',<?php echo $house['subtitle']; ?>,<?php echo $house['images'][0]; ?>,<?php echo $house['description']; ?>,<?php echo $house['rent']; ?>)">加入收藏</button>
